...
% Works with td = 0.01s nu1 = nu2 = 3 tho1=rho2 =-0.8 and 1e-4 tolerances
% Works with td = 0.1s nu1 = nu2 = 3 tho1=rho2 =-0.3 and 1e-4 tolerances
...
nu1 = 3;
rho1 = -0.3;
psi1 = tf(1,1,td)*zeros((nu1+1)*nq1,nq1);
psi1(1:nq1,1:nq1) = tf(1)*eye(nq1);
poly_ = [1,-rho1];
poly = poly_;
for iTF=1:nu1
    psi1((iTF)*nq1+1:(iTF+1)*nq1,1:nq1) = tf(1,poly,td)*eye(nq1);
...
nu2 = 3;
rho2 = -0.3;
psi2 = tf(1,1,td)*zeros((nu2+1)*nq2,nq2);
psi2(1:nq2,1:nq2) = tf(1)*eye(nq2);
poly_ = [1,-rho2];
poly = poly_;
for iTF=1:nu2
    psi2((iTF)*nq2+1:(iTF+1)*nq2,1:nq2) = tf(1,poly,td)*eye(nq2);
    poly = conv(poly,poly_);
end
Anu2 = ss(psi2).A;
Bnu2 = ss(psi2).B;
...
term2 = blkdiag([-X,zeros(length(A));
                 zeros(length(A)),X], ...
...
term3_1 = [eye(length(Anu1)),zeros(length(Anu1),size(Bnu1,2));
           Anu1, Bnu1;
           Cnu1, Dnu1];
term4_1 = blkdiag([-Xnu_1,zeros(length(Anu1));
                   zeros(length(Anu1)), Xnu_1], ...
                  P11_1);

term3_2 = [eye(length(Anu2)),zeros(length(Anu2),size(Bnu2,2));
           Anu2, Bnu2;
           Cnu2, Dnu2];
term4_2 = blkdiag([-Xnu_2,zeros(length(Anu2));
                   zeros(length(Anu2)),Xnu_2], ...
                  P11_2);

% cnstr1 = P11 >= 0;
cnstr2_1 = P12_1 + P12_1' == 0;
cnstr2_2 = P12_2 + P12_2' == 0;
...
% optimize([cnstr5,cnstr6,cnstr7],[],options)
% optimize([cnstr5],[],options)
...
rho2 = -0.3;
psi2 = tf(1,1,td)*zeros((nu2+1)*nq2,nq2);
psi2(1:nq2,1:nq2) = tf(1,1,td)*eye(nq2);
poly_ = [1,-rho2];
poly = poly_;
for iTF=1:nu2
    psi2((iTF)*nq2+1:(iTF+1)*nq2,1:nq2) = tf(1,poly,td)*eye(nq2);
...
psi = blkdiagss(ss(0.2*psi1),ss(0.1*psi2),ss([],[],[],eye(nq3),td), ...
                ss(psi1),ss(psi2),ss([],[],[],eye(np3),td));
[psi, ~] = balreal(psi);

% Realization of [G_qp;I]
G = [T;ss([],[],[],eye(np1+np2+np3),td)];
G = series(G,psi);
[G, ~] = balreal(G);
A = G.A;
B = G.B;
